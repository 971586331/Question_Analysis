...
QString card_create_sql     = "create table AOPA (id INTEGER, subject varchar(500), option_A varchar(200), option_B varchar(200), option_C varchar(200), option_D varchar(200), answer varchar(10))";
QString card_select_max_sql = "select max(id) from AOPA";
QString card_select_min_sql = "select min(id) from AOPA";
QString card_insert_sql     = "insert into AOPA values (?, ?, ?, ?, ?, ?, ?)";
QString card_update_sql     = "update AOPA set card_balance = :card_balance where id = :id";
QString card_select_id_sql  = "select id, subject, option_A, option_B, option_C, option_D, answer from AOPA where id = ?";
QString card_select_all_sql = "select * from AOPA";
QString card_delete_sql     = "delete from AOPA where id = ?";
QString card_clear_sql      = "delete from AOPA";
...
//插入一行数据
void Insert_sql_data(QSqlQuery query, QString insert_sql, Test_Questions_T data)
{
    query.prepare(insert_sql);
    query.addBindValue(data.Title_Number);
    query.addBindValue(data.Subject);
    query.addBindValue(data.Option_A);
    query.addBindValue(data.Option_B);
    query.addBindValue(data.Option_C);
    query.addBindValue(data.Option_D);
